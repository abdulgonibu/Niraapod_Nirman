...
const HotelLists = (props) => {
    ...
    const getHotelLists = async () => {
        const response = await axios(HOTEL_URL);
        if (response.data) {
            if (response.data.results.length > 0) {
                let numrows = response.data.results, smallest = 0, largest = 0
                setHotelLists(response.data.results)
    ...
    const onChangeHotelType = async (event) => {
        ...
        if (URL !== '') {
            hotelURL = hotelURL + URL + "&hotel_types=" + dataString
        } else {
            hotelURL = hotelURL + "?hotel_types=" + dataString
        }
        ...
        <div>
            ...
            <div className="container flex md:flex-row flex-col gap-5 py-10 text-left">
                <div className="shadow  basis-1/4 ">
                    ...
                    <p className="py-2 text-center w-ful">Popular filters for Hotel and Resort </p>
                ...
                <div className=" basis-3/4 ">
                    ...
                    {hotelLists.map((hotel, index) => (
                        <div className="flex md:flex-row flex-col my-10 gap-2 bg-[#f9f9f9]" key={index}>
                            ...
                            <div className="border-r p-1 basis-[40%]">
                                ...
                                <ul className="mt-5 flex gap-1 text-sm">
                                    {hotel.facilities !== '' && hotel.facilities !== null && hotel.facilities.slice(0, 3).map((facility, index) => (
