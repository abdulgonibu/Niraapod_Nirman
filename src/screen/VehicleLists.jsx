...
import { DISTRICT_URL, VEHICLE_FARE_URL, AVAILABLE_DRIVER_URL, SOCKET_IO_URL, SEND_DRIVER_REQUEST_URL, TRIP_URL } from '../constants/api'
...
const VehicleLists = () => {
    ...
    const getVehicesClassWithPrice = async () => {
        ...
        if (response.data) {
            if (response.data.results.length > 0) {
                ...
                let smallest = 0
                let largest = 0, minrat = 0, maxRat = 0
    ...
    const onChangeVehicleClass = async (event) => {
        ...
        let search_URL = '?vehicle_classes=' + dataString.toString()
    ...
    const confirmBoking = async (bookingData) => {
        ...
        if (token && bookingData.vehicle_class.id !== '') {
            ...
            const response = await axios(availableURL, {
                headers: {
    ...
    const requestPostNotFoundDriver = async (data) => {
        const token = localStorage.getItem('token', '');
        const response = await axios.post(TRIP_URL, data, {
            headers: {
                'Authorization': `Token ${token}`
            }
        });
        ...
        if (driversList.length > 0) {
            interval = setInterval(async () => {
                if (i === 0) {
                    ...
                    const token = localStorage.getItem('token', '');
                    const response = await axios.post(SEND_DRIVER_REQUEST_URL, list[0], {
                        headers: {
                            'Authorization': `Token ${token}`
                        }
                    });
    ...
    return (
        <div>
            ...
            <div className="container flex md:flex-row flex-col gap-5 py-10 pb-10 mt-10">
                                ...
                                <div className="grid md:grid-cols-2 bg-gray-100 mb-5" key={index}>
                                    <div className="md:border-r border-gray-400 grid md:grid-cols-2 ">
                                        <div className='relative'>
                                            <img src={vehicle.vehicle_class ? isValidURL(vehicle.vehicle_class.vehicle_class_photo) : null} className="flex w-full text-center mt-12 items-center justify-center p-2" alt="" />
                                        ...
                                        <div className="p-3">
                                            ...
                                            <p className='text-xs text-left'>{vehicle.district_from_details.name + ' - ' + vehicle.district_to_details.name}</p>
